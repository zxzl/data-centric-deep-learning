...
class TrainIdentifyReview(FlowSpec):
    ...
    def crossval(self):
        ...
        for train_index, test_index in kf.split(X):
            ...
            train_dataset = TensorDataset(torch.from_numpy(
                X[train_index]), torch.from_numpy(y[train_index]))
            # pylint: disable=E1101
            test_dataset = TensorDataset(torch.from_numpy(
                X[test_index]), torch.from_numpy(y[test_index]))
            train_dataloader = DataLoader(
                train_dataset, batch_size=config.train.optimizer.batch_size)
            test_dataloader = DataLoader(
                test_dataset, batch_size=config.train.optimizer.batch_size)

            system = SentimentClassifierSystem(config)
            trainer = Trainer(max_epochs=config.train.optimizer.max_epochs)
            # trainer = Trainer(max_epochs=1) #
            trainer.fit(system, train_dataloader, test_dataloader)

            # List of 2d tensors
            probs_tensors = trainer.predict(
                system, test_dataloader, ckpt_path='best')
            for probs_tensor in probs_tensors:
                probs_.extend(probs_tensor.view(-1).tolist())
    ...
    def inspect(self):
        ...
        ranked_label_issues = find_label_issues(self.all_df['label'],  prob,
                                                # Without this parameter returns a boolean mask of label issues.
                                                # https://docs.cleanlab.ai/stable/index.html#find-label-errors-in-your-data
